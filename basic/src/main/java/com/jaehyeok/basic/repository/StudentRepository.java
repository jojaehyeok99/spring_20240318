package com.jaehyeok.basic.repository;
...
// - JPA 기반 Repository를 생성할 때 반드시 상속해야 함
...
extends JpaRepository<StudentEntity, Integer>{
  ...
  // SELECT*FROM student
  // WHERE graduation IS true -> 졸업여부가 true인 사람이 여러개일 경우
  // ORDER BT age DESC;
  List<StudentEntity> findByGraduationOrderByAgeDesc(Boolean graduation);

  // SQL :
  // SELECT*FROM student
  // WHERE student_number = 5 -> 기본키인 studentNumber가 한개로 잡힌 경우
  // AND age > 20;
