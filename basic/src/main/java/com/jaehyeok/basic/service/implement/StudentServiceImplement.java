...
import com.jaehyeok.basic.entity.StudentEntity;
import com.jaehyeok.basic.repository.StudentRepository;
...
public class StudentServiceImplement implements StudentService{
  ...
  public ResponseEntity<String> postStudent(PostStudentRequestDto dto) {
    ...
    // 패스워드 암호화 작업
    String password = dto.getPassword(); // 사용자가 등록한 패스워드를 가져옴
    String encodedPassword = passwordEncoder.encode(password); // 패스워드를 암호화 함

    dto.setPassword(encodedPassword); // 위에서 암호화된 패스워드로 변경
  ...
  public ResponseEntity<String> signIn(SignInRequestDto dto) {
    ...
    try{
      ...
      // 사용자가 입력한 패스워드와 암호화된 패스워드가 매치되는지 확인하는 작업
