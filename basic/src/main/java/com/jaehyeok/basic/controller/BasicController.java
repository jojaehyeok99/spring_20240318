package com.jaehyeok.basic.controller;
...
import com.jaehyeok.basic.service.implement.BasicServiceImplement;
...
public class BasicController {
  ...
  // 생성자를 통한 의존성 주입을 spring에서 권장(의존성이 주입되지 않은 경우가 존재할 수 없기 때문 - 안전성 문제 해결)
  ...
  // HTTP GER Method : 클라이언트가 서버로부터 데이터를 받기를 원할 때 사용하는 메서드
  ...
  // HTTP POST Method : 클라이언트가 서버에 데이터를 작성하길 원할 때 사용하는 메서드 
  // EX) 회원가입

  // @PostMapping() : RequestMapping 기능을 Post Mathod에 한정시킨 것 (가독성 + 안정성)
  @PostMapping("/apple")
  ...
  // @PutMapping() : RequestMapping 기능을 Put Mathod에 한정시킨 것 (가독성 + 안정성)
  @PutMapping("/apple")
  ...
  //@PatchMapping() : RequestMapping 기능을 Patch Mathod에 한정시킨 것 (가독성 + 안정성)
  @PatchMapping("/apple")
  public String patchApple(){
    return "Patch Mapping으로 만든 메서드";
  }

  // HTTP DELETE Method : 클라이언트가 서버에 있는 리소스 삭제하고 싶을 때 사용하는 메서드

  //@DeleteMapping() : RequestMapping 기능을 Delete Mathod에 한정시킨 것 (가독성 + 안정성)
  @DeleteMapping("/apple")
