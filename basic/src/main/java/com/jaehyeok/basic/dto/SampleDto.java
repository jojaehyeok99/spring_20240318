package com.jaehyeok.basic.dto;
...
@AllArgsConstructor
@ToString
//Request Body 로 받는 데이터에 대한 유효성 검사를 validation 의존성을 추가 해야 함
public class SampleDto {
  ...
  // @Length : 문자열일 경우 문자열 길이의 최소 최대를 지정(null 값이면 검사 안함)
  @Length(min=5, max=10)
  @NotNull
  private String length;

  @NotNull
  // @MAX, @Min : 최댓값과 최솟값을 지정
  @Max(100)
  @Min(0)
  ...
  // @Pattern : 정규식을 이용해 문자열의 패턴 검사를 진행
  // 정규식(Regular Expression) : 특정한 규칙을 가진 문자열을 가진 문자열 집합을 표현하기 위해 쓰이는 형식
  // 전화번호, 주민등록번호, 이메일, URL, 비밀번호, 영어/한글/숫자로만된 문자열과 같이 특정 형식의 값을 검증해야 할 때 사용
